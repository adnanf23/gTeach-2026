...
import {
  ArrowLeft, LayoutGrid, Users, BookOpen, Search,
  GraduationCap, UserCheck, Award, CalendarDays, Hash,
  School, Loader2, UserMinus, BookMarked, Layers,
} from "lucide-react"
...
function Avatar({ name = "?", size = "sm", colorClass }) {
  ...
  const bg = colorClass || palette[(name || "?").charCodeAt(0) % palette.length]
  const sz = {
    xs: "w-6 h-6 text-[9px]",
  ...
  if (!user) {
    return (
      <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-zinc-50 border border-dashed border-zinc-200">
        ...
        <div>
          <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-widest">{roleLabel}</p>
          <p className="text-[13px] text-zinc-400 italic mt-0.5">Belum ditetapkan</p>
        </div>
      </div>
    )
  }
  return (
    <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-white border border-zinc-100 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-widest">{roleLabel}</p>
        <p className="text-[13px] font-semibold text-zinc-900 truncate mt-0.5">{user.nama_lengkap || "-"}</p>
...
function Skeleton({ className }) {
  return <div className={cn("animate-pulse bg-zinc-100 rounded-xl", className)} />
}
...
function StatCard({ icon: Icon, label, value, colorClass = "bg-blue-50 text-blue-600" }) {
  return (
    <div className="bg-white border border-zinc-100 rounded-2xl p-4 shadow-sm flex items-center gap-3 hover:shadow-md transition-shadow">
      ...
      <div>
        <p className="text-[18px] font-bold text-zinc-900 leading-none">{value}</p>
...
export default function DetailKelasPage() {
  ...
  useEffect(() => {
    ...
    pb.collection("kelas").getOne(id, {
      ...
      .catch(err => { console.error(err); setKelas(null) })
  ...
  const fetchMapel = useCallback(async () => {
    ...
    try {
      ...
      const spesifik = await pb.collection("mata_pelajaran").getFullList({
        filter: `spesifik_kelas_id ~ "${id}"`, requestKey: null,
      }).catch(() => [])
      const umum = await pb.collection("mata_pelajaran").getFullList({
        filter: `target_tingkat ~ "${tingkat}"`, requestKey: null,
      }).catch(() => [])
      const map = {}
      ;[...spesifik, ...umum].forEach(m => { map[m.id] = m })
      setMapel(Object.values(map))
      setFetchedMapel(true)
    } catch (err) { console.error(err) }
    finally { setLoadMapel(false) }
    ...
    return (
      <div className="min-h-screen bg-zinc-50/60 flex items-center justify-center p-6">
        <div className="text-center space-y-3">
          ...
          <p className="text-zinc-700 font-semibold text-base">Kelas tidak ditemukan</p>
          <p className="text-zinc-400 text-sm">ID kelas tidak valid atau sudah dihapus.</p>
          <button
            onClick={() => router.back()}
  ...
  const heroGradient = gradientMap[String(kelas.tingkat)] || "from-blue-600 to-blue-800"
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-5">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className="text-white/60 text-[10px] font-bold uppercase tracking-widest">
                    {TINGKAT_LABEL[String(kelas.tingkat)] || `Tingkat ${kelas.tingkat}`}
                  </span>
                  {tahunAjaran && (
                ...
                <h1 className="text-[26px] font-bold tracking-tight leading-none">{kelas.nama_kelas}</h1>
                <p className="text-white/40 text-[11px] font-mono mt-1">{kelas.id}</p>
              </div>
            </div>

            {/* Right: kapasitas */}
            <div className="shrink-0 bg-white/10 rounded-2xl p-4 backdrop-blur-sm ring-1 ring-white/10 min-w-[130px]">
              <p className="text-white/60 text-[10px] font-bold uppercase tracking-widest mb-2">Kapasitas</p>
              <div className="flex items-end gap-1">
                <span className="text-4xl font-bold leading-none tabular-nums">
                  {jumlahSiswa !== null ? jumlahSiswa : <span className="text-2xl opacity-50">—</span>}
                </span>
                <span className="text-white/50 text-base mb-0.5">/{kapasitas}</span>
              </div>
                  ...
                  <div className="h-1.5 bg-white/15 rounded-full overflow-hidden">
                    <div
                      className={cn(
                        ...
                        pct >= 95 ? "bg-red-300" : pct >= 75 ? "bg-amber-300" : "bg-emerald-300"
                      )}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <p className="text-white/40 text-[10px] mt-1 text-right">{pct}%</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="relative mt-5 pt-5 border-t border-white/10 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                {user ? (
                  <>
                    <Avatar name={user.nama_lengkap || ""} size="sm" colorClass="bg-white/20 text-white" />
                    <div>
                      <p className="text-white/50 text-[10px] font-semibold uppercase tracking-widest">{label}</p>
                      <p className="text-white font-semibold text-[13px]">{user.nama_lengkap}</p>
                    </div>
                  </>
                ) : (
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-white/50 text-[10px] font-semibold uppercase tracking-widest">{label}</p>
                      <p className="text-white/40 italic text-[13px]">Belum ditetapkan</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            >
              ...
              {t.id === "siswa" && fetchedSiswa && (
                <span className={cn(
                  ...
                  tab === "siswa" ? "bg-blue-100 text-blue-600" : "bg-zinc-200 text-zinc-500"
                )}>{siswa.length}</span>
              )}
              {t.id === "mapel" && fetchedMapel && mapel.length > 0 && (
                <span className={cn(
                  "px-1.5 py-0.5 text-[10px] font-bold rounded-full",
                  tab === "mapel" ? "bg-violet-100 text-violet-600" : "bg-zinc-200 text-zinc-500"
                )}>{mapel.length}</span>
              )}
            </button>
          ))}
        ...
        {tab === "overview" && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatCard icon={Layers}    label="Tingkat"        value={kelas.tingkat}                              colorClass="bg-blue-50 text-blue-600"    />
              <StatCard icon={Users}     label="Jumlah Siswa"   value={jumlahSiswa !== null ? jumlahSiswa : "—"}   colorClass="bg-violet-50 text-violet-600" />
              <StatCard icon={School}    label="Sisa Slot"      value={jumlahSiswa !== null ? kapasitas - jumlahSiswa : "—"} colorClass="bg-emerald-50 text-emerald-600" />
              <StatCard icon={BookMarked}label="Mata Pelajaran" value={fetchedMapel ? mapel.length : "—"}          colorClass="bg-amber-50 text-amber-600"   />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <PersonCard user={walikelas}  roleLabel="Wali Kelas"      icon={UserCheck} />
                <PersonCard user={pendamping} roleLabel="Guru Pendamping" icon={UserCheck} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl shadow-sm overflow-hidden">
                {[
                  ...
                  ["Tingkat",         TINGKAT_LABEL[String(kelas.tingkat)] || `Tingkat ${kelas.tingkat}`],
                  ["Wali Kelas",      walikelas?.nama_lengkap + ' (' + kelas.expand?.walikelas_id?.username + ') '|| "-"],
                  ["Guru Pendamping", pendamping?.nama_lengkap  + ' (' + kelas.expand?.pendamping_id?.username + ') ' || "-"],
                  ["Tahun Ajaran",    tahunAjaran?.nama || tahunAjaran?.tahun || kelas.tahun_ajaran_id || "-"],
                  ["Dibuat",          kelas.created ? new Date(kelas.created).toLocaleString("id-ID", { dateStyle:"long", timeStyle:"short" }) : "-"],
                  ["Diperbarui",      kelas.updated ? new Date(kelas.updated).toLocaleString("id-ID", { dateStyle:"long", timeStyle:"short" }) : "-"],
                ].map(([k, v], i) => (
                  <div
                    key={k}
                    className={cn(
                  ...
                  >
                    <span className="w-36 shrink-0 text-zinc-400 font-medium pt-px">{k}</span>
                    <span className="text-zinc-700 font-mono break-all">{v}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <>
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Badge variant={pct >= 95 ? "red" : pct >= 75 ? "amber" : "green"}>
                ...
                {siswa.length === 0 ? (
                  <div className="py-20 flex flex-col items-center justify-center gap-2 text-zinc-400">
                    ...
                    <p className="font-semibold text-zinc-500 text-[14px]">Belum ada siswa</p>
                    <p className="text-[12px] text-zinc-400">Belum ada siswa yang terdaftar di kelas ini</p>
                  </div>
                ) : (
                  <div className="bg-white border border-zinc-100 rounded-2xl overflow-hidden shadow-sm">
                      ...
                      <table className="w-full text-[13px] min-w-[500px]">
                        <thead>
                          <tr className="bg-zinc-50 border-b border-zinc-100">
                            <th className="px-5 py-3.5 text-left text-[10px] font-bold text-zinc-400 uppercase tracking-widest w-10">#</th>
                            <th className="px-5 py-3.5 text-left text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Nama Siswa</th>
                            <th className="px-5 py-3.5 text-left text-[10px] font-bold text-zinc-400 uppercase tracking-widest">NIS</th>
                            <th className="px-5 py-3.5 text-left text-[10px] font-bold text-zinc-400 uppercase tracking-widest">NISN</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-50">
                          {filteredSiswa.length === 0 ? (
                            <tr>
                              <td colSpan={4} className="px-5 py-10 text-center text-zinc-400 text-[13px] italic">
                                Tidak ada siswa yang sesuai pencarian
                              </td>
                            </tr>
                          ) : filteredSiswa.map((s, i) => (
                            <tr key={s.id} className="hover:bg-zinc-50/70 transition-colors">
                              <td className="px-5 py-3.5 text-zinc-400 text-[11px] tabular-nums">{i + 1}</td>
                              <td className="px-5 py-3.5">
                                <div className="flex items-center gap-2.5">
                                  <Avatar name={s.nama_siswa || "?"} size="xs" />
                                  <span className="font-semibold text-zinc-800">{s.nama_siswa || "-"}</span>
                                </div>
                              </td>
                              <td className="px-5 py-3.5 font-mono text-[12px] text-zinc-500">
                                {s.nis || <span className="text-zinc-300">—</span>}
                              </td>
                              <td className="px-5 py-3.5 font-mono text-[12px] text-zinc-500">
                                {s.nisn || <span className="text-zinc-300">—</span>}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                    ...
                    <div className="px-5 py-3 border-t border-zinc-50 bg-zinc-50/50 text-[12px] text-zinc-400 flex justify-between items-center">
                      <span>
                        Menampilkan <strong className="text-zinc-600">{filteredSiswa.length}</strong> dari{" "}
                        <strong className="text-zinc-600">{siswa.length}</strong> siswa
                      </span>
                      <span>
                        <strong className="text-zinc-600">{kapasitas - siswa.length}</strong> slot tersisa
                      </span>
                    </div>
                  </div>
                )}
          ...
          <div className="space-y-4">
            {loadMapel ? (
              ...
              <div className="py-20 flex flex-col items-center justify-center gap-2 text-zinc-400">
                ...
                <p className="font-semibold text-zinc-500 text-[14px]">Belum ada mata pelajaran</p>
                <p className="text-[12px] text-zinc-400 text-center max-w-xs">
                  Mapel akan muncul jika ada{" "}
                  <code className="text-[11px] bg-zinc-100 px-1 py-0.5 rounded font-mono">target_tingkat</code>{" "}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {mapel.map((m, i) => {
                    const accents = [
                      ...
                      { bg: "bg-violet-50",  icon: "bg-violet-100 text-violet-600" },
                      { bg: "bg-emerald-50", icon: "bg-emerald-100 text-emerald-600" },
                      { bg: "bg-amber-50",   icon: "bg-amber-100 text-amber-600"   },
                    ...
                    return (
                      ...
                      >
                        <div className={cn(
                          ...
                          {(m.kode_mapel || m.nama_mapel || "?").slice(0, 3).toUpperCase()}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] font-mono text-zinc-400">{m.kode_mapel || "-"}</p>

                          {Array.isArray(m.target_tingkat) && m.target_tingkat.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {m.target_tingkat.map(t => (
                                <Badge
                                  key={t}
                                  variant={String(t) === String(kelas.tingkat) ? "blue" : "gray"}
                                >
