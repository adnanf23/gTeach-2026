...
export default function WalikelasLayout({ children }) {
  ...
  // Logika penentuan activeKey
  const getActiveKey = () => {
    const exactMatch = NAV.find((n) => pathname === n.href);
    if (exactMatch) return exactMatch.key;

    if (pathname === "/walikelas/profile") return "profile";

    const nestedMatch = NAV.find(
      (n) => n.href !== "/walikelas" && pathname.startsWith(n.href),
    );
  ...
  const isProfileActive = pathname === "/walikelas/profile";
